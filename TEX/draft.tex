...
\textit{Yes} means that the tool supports signing of certificate signing request. \textit{No} if it does not support signing of certificate signing requests.
...
\textit{Yes} - Possibility of choosing how long the certificate will be valid. Done by certificate authority. \textit{No} - If there is no option to change how long the certificate will be valid, so the duration is a fixed ammount.
