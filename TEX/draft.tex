...
\paragraph{PEM encoded certificates (.pem$|$.cer$|$.crt):}

  \verb+openssl x509 -in sample_cert.extention -text -noout+

\paragraph{DER encoded certificates (.der):}

  \verb+openssl x509 -in certificate.der -inform der -text -noout+

\paragraph{Importing PEM or DER encoded keys or certificates into PKCS#12 file:}
